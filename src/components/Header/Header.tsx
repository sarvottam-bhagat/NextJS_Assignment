...
const IconButton: React.FC<IconButtonProps> = ({ icon: Icon, label, extraIcon: ExtraIcon, iconColor, textColor, size }) => (
  <button className={`flex items-center space-x-2 py-1 px-3 shadow-sm rounded-sm cursor-pointer ${textColor || "text-[#5D6876]"}`}>
...
const Header: React.FC = () => {
  return (
    <header className="flex justify-between w-full h-fit text-sm font-semibold p-4 border-b border-slate-200">
